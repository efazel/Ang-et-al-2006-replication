%% this code performs the same estimting process as Ang et al. 2006
% To replicate the first table we can do the following:
% either regress the excess returns on market factor and NC or NS
% (separately or together)
%or just regress on NC and NS 
clear;
clc;
%for now just regress on NS and NC and Mrk and sort single way 

load('raw_CS_return.mat'); %monthly return for common stock from 1976-2018
load('raw_CS_cap.mat');%

load('mrkt_96_2013.mat'); %market factor
load('RF_96_2013.mat'); %risk free rate

load('Concentration2.mat'); %loading factors
load('Concentration1.mat');

load('VIXmonthly.mat') %load VIX
%inno_VIX = diff(VIX);
...
permno = raw_CS_return(1,:); %permno header for each col
cs_rtn = raw_CS_return(242:458,:); %common stock return to Jan 2014-one month ahead
cs_cap = raw_CS_cap(242:458,:); %common stock market cap to Jan 2014-one month ahead
%% test for NC in innovation
inno_NC = diff(CONCENTRATION);
cs_rtn = raw_CS_return(243:457,:); %common stock return
cs_cap = raw_CS_cap(243:457,:); %common stock market cap
...
w = 24; %window length
k = size(cs_rtn,1);
N = k - w ; %number of intervals/number of portfolios
...
num_NaN_t_1 = isnan(r_t_1); %check for NaN in r_t_1
total_num_NaN_t_1 = sum(num_NaN_t_1);
NaN_rmv = find(total_num_NaN_t_1 >= 3);
r_t_1(:,NaN_rmv) = []; %remove them
P_t_1(:,NaN_rmv) = [];
...
% we need
%R_t_1 & R_t_cell , MARKET and RF and a column of one each time
% and also the factors: for now: concentration with 2 version
...
%in sortedNC, first row corresponding permno, 2nd row index of the firm,
%3rd its sorted return
% creating quintile portfolios for each period N=192
qnt = 5;
for i=1:N
    
totalsorted = sortedNC{i}; %total sorted firms for the window
q1_num = round(size(totalsorted,2)/qnt);%dimension of each quitile (num of firms in each)
q5_num = q1_num; 
q2_num = q1_num;
q4_num = q1_num;
...
%and market cap in period t in R_t_cell. so we can creat the value eighted
%returns.
